...
class Router(object):
    def __init__(self, transport, localfile=""):
        ...
        self.weights = lambda lat,lng: sp.array([0]*len(lat))
    ...
    def distance(self, n1, n2):
        ...
        path = lambda t: 180/sp.pi*sp.array([lat1 + (lat2 - lat1)*t, lon1 + (lon2 - lon1)*t])
        points = sp.linspace(0., 1., 100)
        factor = (10**8)*sp.integrate.trapz(self.weights(*(path(points))), points)
        return math.asin(math.sqrt(d)) * 12742 *(1+ factor)
